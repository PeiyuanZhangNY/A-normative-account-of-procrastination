...
for nMonte = 1: NMonte
    ...
    [temp2,] = HyperbolicDiscountingFun(TAll(nMonte),k,JAll(nMonte),c1All(nMonte),lambdaAll(nMonte),alphaAll(2,nMonte),betaAll(nMonte),k_DR(nMonte)); 
...
%%%%%% find(deltatotalcost>0). bidirectional, mostly <=0
b = Nnetutility(:,1)-Nnetutility(:,2);
% % find(b>0) % if no, then net utility is larger for larger alpha
% % % bidirectional, mostly <=0. 

% save('Hyperbolic_alphaMonteCarlo.mat')
